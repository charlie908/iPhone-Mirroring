...
struct ContentView: View {
    @AppStorage("directPeer") private var serverHost = "direct"
    ...
    private var connectionSheet: some View {
        VStack(alignment: .leading, spacing: 22) {
            Label("Connexion directe", systemImage: "point.3.connected.trianglepath.dotted")
                .font(.title2.weight(.semibold))
            Text("L’iPhone est découvert automatiquement à proximité, sans routeur ni connexion Internet.")
                .foregroundStyle(.secondary)
            Label(
                controls.isConnected ? "iPhone connecté" : "En attente de l’iPhone",
                systemImage: controls.isConnected ? "checkmark.circle.fill" : "antenna.radiowaves.left.and.right"
            )
            .foregroundStyle(controls.isConnected ? .green : .secondary)
            HStack {
                Spacer()
