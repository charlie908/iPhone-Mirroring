...
struct NativeH264StreamView: UIViewRepresentable {
    ...
    final class Coordinator {
        ...
        func connect(host: String) {
            guard connectedHost.isEmpty else { return }
            disconnect()
            connectedHost = "direct"
            status.state = .loading
            DirectPeerTransport.shared.videoHandler = { [weak self] message in
                self?.consume(message)
            }
            DirectPeerTransport.shared.start()
            logger.info("Waiting for direct iPhone H264 stream")
            print("iPhone Direct: waiting for peer-to-peer video")
        }
